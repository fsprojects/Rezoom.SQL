...
module Names =
    ...
    let impure = "impure"
    let unsafe_coerce = "unsafe_coerce"
    let unsafe_inject_raw = "unsafe_inject_raw"

/// Functions that are supported by EVERY database. Surprisingly there aren't many of these.
let common =
    [|  func Names.abs [ numeric (infect a') ] a'
        func Names.coalesce [ nullable a'; vararg (nullable a'); infect a' ] a'
    |]
...
let builtins =
    [|  // Used to prevent queries from being assumed idempotent, even though they otherwise seem to be.
        ErasedFunction(Name(Names.impure), infect a', a', idem = false) :> FunctionType
        ...
        erased Names.nullable (nullable a') (nullable a')
        ...
        erased Names.unsafe_coerce (infect scalar) scalar
        erased Names.unsafe_inject_raw rawsql any
    |]
